...
#if LIGATURES_PATCH
static inline void xresetfontsettings(uint32_t mode, Font **font,
                                      int *frcflags);
#endif // LIGATURES_PATCH
...
static void setsel(char *, Time);
    ...
    BACKGROUND_IMAGE_PATCH && BACKGROUND_IMAGE_RELOAD_PATCH
...
int xloadfont(Font *f, FcPattern *pattern) {
  ...
  f->width = DIVCEIL(extents.xOff > 18 ? extents.xOff / 3 : extents.xOff,
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len,
  ...
  // gradient
  bg->color.alpha =
      grad_alpha * 0xffff * (win.h - y * win.ch) / win.h + stat_alpha * 0xffff;
// uncomment to invert the gradient
// bg->color.alpha = grad_alpha * 0xffff * (y*win.ch) / win.h + stat_alpha *
// 0xffff;
  ...
  if (url_draw && y >= url_y1 && y <= url_y2) {
    ...
    if (x + charlen > x1 && x <= x2) {
      ...
      XftDrawRect(xw.draw, fg, xu,
                  winy + win.cyo + dc.font.ascent * chscale + 2, wu, 1);
#else
...
#else
                borderpx + (cx + 1) * win.cw - 1, borderpx + cy * win.ch,
#endif // ANYSIZE_PATCH
                1, win.ch - 1);
    XftDrawRect(xw.draw, &drawcol,
#if ANYSIZE_PATCH
...
#else
                borderpx + cx * win.cw, borderpx + (cy + 1) * win.ch - 1,
#endif // ANYSIZE_PATCH
                win.cw, 1);
  }
}
...
void xdrawline(Line line, int x1, int y1, int x2) {
  ...
  specs = xw.specbuf;
  for (i = 0; i < j; i++) {
    xdrawglyphfontspecs(specs, seq[i].base, seq[i].numspecs, seq[i].ox, y1,
                        DRAW_FG, seq[i].charlen);
  ...
  for (im = term.images; im; im = next) {
    ...
    if (!im->pixmap) {
      ...
      if (win.cw == im->cw && win.ch == im->ch) {
        ...
        XPutImage(xw.dpy, (Drawable)im->pixmap, dc.gc, &ximage, 0, 0, 0, 0,
                  width, height);
        if (im->transparent)
          im->clipmask =
              (void *)sixel_create_clipmask((char *)im->pixels, width, height);
      } else {
        ...
        scaled = imlib_create_cropped_scaled_image(0, 0, im->width, im->height,
                                                   width, height);
        ...
        if (im->transparent)
          im->clipmask = (void *)sixel_create_clipmask(
              (char *)imlib_image_get_data_for_reading_only(), width, height);
        imlib_free_image_and_decache();
      }
    }
    ...
    for (del = 1, x1 = im->x; x1 < xend; x1 = x2) {
      ...
      if (mode) {
        XCopyArea(xw.dpy, (Drawable)im->pixmap, xw.buf, gc,
                  (x1 - im->x) * win.cw, 0,
                  MIN((x2 - x1) * win.cw, width - (x1 - im->x) * win.cw),
                  height, bw + x1 * win.cw, desty);
        del = 0;
      }
    }
    if (im->clipmask)
